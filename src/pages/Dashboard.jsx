...
import { Menu, TrendingUp, TrendingDown, ArrowUpRight, Hash } from "lucide-react";
import {
...
export default function Dashboard() {
  const location = useLocation();  // leer navegación

  const [sidebarAbierto, setSidebarAbierto] = useState(false);
  const [tagActivo,      setTagActivo]      = useState(
    location.state?.tag || "python"
  );
  ...
  // ← Sincronizar cuando viene desde /tecnologias
  useEffect(() => {
    if (location.state?.tag) {
      setTagActivo(location.state.tag);
    }
  }, [location.state?.tag]);

  // ← Cargar datos cuando cambia el tag
  useEffect(() => {
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="dashboard-layout">
      <Sidebar
        ...
        categoriaActiva={tagActivo}
        onCategoriaChange={(tag) => setTagActivo(tag)}
      />
        ...
        <div className="top-header">
          <div className="top-header-left">
            ...
            <div className="top-header-title">
              <h1>PULSO TECNOLÓGICO</h1>
              <span>Stack Overflow Analytics Platform</span>
            </div>
          </div>
        </div>
                  ...
                  <div className="metrica-exp-item">
                    ...
                    <span className="metrica-exp-desc">
                      ...
                      {clasificacion.pendiente > 0
                        ? " Valor positivo = crece en preguntas por mes."
                        : " Valor negativo = decrece en preguntas por mes."
                      }
                    </span>
                  </div>
                  <div className="metrica-exp-item">
                ...
                <div className="section-header">
                  <div>
                    <h2 className="section-titulo">
                      Series de tiempo — Evolución mensual
                    </h2>
